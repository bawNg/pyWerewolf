...
class WerewolfBot(SingleServerIRCBot):
	...
	def on_all_events(self, c, e):
		if e.eventtype() != "all_raw_messages":
			print "%s %s %s %s" % (e.source(), e.eventtype().upper(), e.target(), e.arguments())

	def on_nicknameinuse(self, c, e):
		for n in config.irc.nickname:
			if n != c.get_nickname(): c.nick(n); break
		if c.get_nickname() != config.irc.nickname[0]: c.nick(config.irc.nickname[0])
	...
	def on_privmsg(self, c, e):
		msg = e.arguments()[0]
		if (msg[:1] == "!") and (msg.find("!",1,4) == -1) and (msg.find("?",1,4) == -1): msg = msg[1:]
	...
	def on_pubmsg(self, c, e):
		...
		elif (msg[:1] == "!") and (msg.find("!",1,4) == -1) and (msg.find("?",1,4) == -1):
